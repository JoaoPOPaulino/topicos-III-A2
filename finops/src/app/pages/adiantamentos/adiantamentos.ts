...
export class Adiantamentos {
  ...
  dados = [
    ...
    { id: 1002, nome: "Ana Costa", desc: "Compra materiais", valor: "$ 320.00", moeda: "USD", data: "2025-02-02", status: "Rejeitado" },
  ...
  filtrar() {
    let filtrados = this.dados.filter(item => {
      const texto = (item.nome + " " + item.desc + " " + item.valor + " " + item.status).toLowerCase();
  ...
  atualizarPaginacao() {
    ...
    let filtrados = this.dados.filter(item => true);
